...
export function ManifestoSection() {
  ...
  /** Torres por baixo (opcional); mesmo padrão no desktop e no mobile — tudo em fluxo normal, sem sticky. */
    ...
    >
      ...
      {desktopLg ? (
        <div className="container min-w-0 pb-16">
          {/**
           * Desktop em fluxo normal (sem sticky / palco 160vh): imagem e texto sobem juntos.
           * Antes: sticky + overlay absolute no mesmo pai → texto “escorria” sobre a foto fixa.
           */}
          <div
            className={cn(
              "relative mb-12 w-full max-w-full overflow-hidden rounded-sm bg-forest-dark",
              "min-h-[520px] h-[78vh] md:min-h-[560px] md:h-[min(88vh,920px)]"
            )}
          >
            {!trackImgFailed && (
              <img
            ...
            />
            <div className="absolute inset-x-0 bottom-0 z-[3] px-6 pb-10 pt-24 md:px-8 md:pb-12">
              <div className="mx-auto flex min-w-0 max-w-[min(100%,72rem)] flex-col gap-4 sm:flex-row sm:items-end sm:gap-8">
                <p className="max-w-full min-w-0 flex-1 text-paper/90 text-[1.0625rem] font-light leading-[1.65] md:max-w-2xl">
                ...
                <span className="min-w-0 max-w-full shrink-0 text-paper/60 text-[0.65rem] uppercase tracking-[0.2em] sm:max-w-[min(100%,14rem)] sm:text-right sm:text-[0.7rem] sm:tracking-[0.25em]">
                  {locationChip}
                </span>
              </div>
            </div>
          ...
          <div className="mb-8 grid min-w-0 grid-cols-1 gap-10 gap-y-12 lg:grid-cols-12 lg:gap-x-12 [&>*]:min-w-0">
            <motion.div
